...
* Added missing event timezone inputs to `r34ics_date()` calls in the `R34ICS2::event_description_html()` method and `r34ics_multiday_date_label()` function. Without timezones, date ranges for multi-day events could be one day "off" in the event descriptions and labels after the aforementioned refactoring of `r34ics_date()`.
* Fixed incorrect date display issues in `R34ICS2::event_description_html()`. _Note: Version 11.5.4 was withdrawn due to this issue, which was discovered a few minutes after its release._
...
* Added `r34ics_comparison_string()` function to process strings for comparison purposes. Used by features added to `R34ICS2::r34ics_display_calendar_exclude_event()` in 11.5.2 to filter events by category, accounting for possible formatting issues with accented letters.
 ...
 * Modified the `event_description_html()`, `event_label_html()` and `event_sublabel_html()` methods of `R34ICS2` to return empty strings rather than null or false values when empty.
...
* Bug fix: Reverted change from 11.4.0 that made `month` the default fallback value for `$view` if it is not passed into `R34ICS2::shortcode_defaults_merge()` method because this caused incorrect default values to override custom values in some cases.
...
* AJAX:
  ...
  * Added `r34ics_shortcode_defaults_merge` filter, to allow `R34ICS2::shortcode_defaults_merge()` to be used as a general method for retrieving the full list of valid shortcode attributes and their default values.
* Miscellaneous:
...
* Added `class_exists()` check to `R34ICS2` class.
...
* Miscellaneous:
  * Added `R34ICS2::shortcode_feed_array_params` property and logic to split pipe-delimited, feed-related parameters into arrays for easier logic elsewhere, plus related functionality, including `r34ics_shortcode_feed_array_params` filter. (This is used by `feedlabelindesc` and may be used by other future enhancements.)
...
* Miscellaneous:
	* Added check for `R34ICS2` class already being defined, to prevent a potential fatal error on plugin activation.
  ...
  * Updated protected properties of the `R34ICS2` class.
* Hotfix:
...
* Added `R34ICS2::get_list_style_views()` and `r34ics_list_style_views` filter to allow external addition of list-style views for certain conditional logic.
* Performance improvements: Narrowed rough parsed date ranges defined in `R34ICS2::display_calendar_date_range()` to reduce memory usage and processing time for calendars with a large number of recurring events.
...
* Developer:
  * Added two new actions: `r34ics_shortcode_before_display_calendar` and `r34ics_shortcode_after_display_calendar`. These actions fire off in `R34ICS2::shortcode()` before and after the call to `R34ICS2::display_calendar()`.
  * Corrected `add_action()` to `add_filter()` for `r34ics_display_calendar_exclude_event` in `class-r34ics.php`.
* Miscellaneous:
  * Removed logic to run `gzdecode()` in `R34ICS2::_url_get_contents()` because `wp_remote_get()` already does that. Eliminates appearance of numerous related PHP warnings in log files.
...
* Code Refactoring:
  * Replaced previous cURL and fopen logic with `wp_remote_get()` in `R34ICS2::_url_get_contents()` method. Correspondingly, deprecated the `curlopts` and `method` parameters. _Note: The second and fourth parameters of the protected method `R34ICS2::_url_get_contents()` were **removed** rather than deprecated. No external plugins should be using this method other than ICS Calendar Pro, which has been correspondingly updated._
...
* Miscellaneous:
  ...
  * Modified `R34ICS2::event_description_html()` to handle `$event['start_time']` and `$event['end_time']` as alternatives to `$event['start']` and `$event['end']`.
