...
$days_of_week = $R34ICS2->get_days_of_week($args['columnlabels']);
			...
			<tbody><tr>
				...
				foreach (array_keys((array)$ics_data['events']) as $year) {
					for ($m = 1; $m <= 12; $m++) {
						...
						$first_dow = $R34ICS2->first_dow($m.'/1/'.$year);
											...
											foreach ((array)$events as $event) {
												...
												if ($time == 'all-day') {
													...
													?>>
														...
														// Event label (title)
														echo wp_kses_post($R34ICS2->event_label_html($args, $event, (!empty($has_desc) ? array('has_desc') : null)) ?: '');

														// Sub-label
														echo wp_kses_post($R34ICS2->event_sublabel_html($args, $event, null) ?: '');

														// Description/Location/Organizer
														echo wp_kses_post($R34ICS2->event_description_html($args, $event, (empty($args['toggle']) ? array('hover_block') : null), $has_desc) ?: '');
														?>
													</li>
													<?php
												}
