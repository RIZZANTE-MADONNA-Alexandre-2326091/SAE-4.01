...
class InformationView extends View
{
    ...
    public function displaySlide(string $title, string $content, string $type, bool $adminSite = false): void
    {
		...
		else if ($type == 'YTvideosh')
		{
			...
			echo '<iframe id="" class="videosh" src="' . $link . '?autoplay=1&loop=1&playlist=' . substr($link,30) . '&mute=1&controls=0&disablekb=1&showinfo=0&enablejsapi=1"
		...
		else if ($type == 'YTvideow')
		{
			...
			echo '<iframe id="" class="videow" src="' . $link . '?autoplay=1&loop=1&playlist=' . substr($link,30) . '&mute=1&controls=0&disablekb=1&showinfo=0&enablejsapi=1"
